...
def check_stock() -> dict:
    try:
        # Cek stok via halaman service smspva
        url = f"https://smspva.com/service/whatsapp"
        headers = {"User-Agent": "Mozilla/5.0"}
        r = requests.get(url, headers=headers, timeout=10)

        # Coba endpoint API publik
        api_url = "https://smspva.com/priemnik.php"
        params = {
            "metod":   "get_count_new",
            "country": COUNTRY,
            "service": SERVICE,
        ...
        # Coba parse response
        text = r2.text.strip()
        logging.info(f"Raw response: {text[:100]}")
        
        import json
        try:
            data = json.loads(text)
        except:
            # Kalau bukan JSON, coba parse manual
            if "online" in text.lower():
                return {"success": True, "online": 1, "total": 1}
            return {"success": False, "msg": f"Non-JSON: {text[:50]}"}
